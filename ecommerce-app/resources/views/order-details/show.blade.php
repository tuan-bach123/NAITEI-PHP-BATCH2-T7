						...
						<div class="space-y-4 p-6">
							<div class="flex items-center gap-6">
								<a href="/products/{{ $orderItem->product_id }}" class="h-14 w-14 shrink-0">
									<img class="h-full w-full dark:hidden"
										src="{{ Vite::asset('storage/app/' . $orderItem->product->image_url) }}"
										alt="imac image" />
								</a>
							...
							<div class="flex items-center justify-between gap-4">
								<p class="text-sm font-normal text-gray-500 dark:text-gray-400"><span
									...
									{{ $orderItem->product_id }}</p>

								<div class="flex items-center justify-end gap-4">
									<p class="text-base font-normal text-gray-900 dark:text-white">x{{ $orderItem->qty }}
									</p>

									<p class="text-xl font-bold leading-tight text-gray-900 dark:text-white">
										${{ $orderItem->product->price }}</p>
								</div>
							</div>
							<div class="w-full mx-auto">
								<form action="{{ route('review.store', ['order_item_id' => $orderItem->id]) }}"
									method="POST" class="w-full flex items-center gap-4">
									@csrf
									@method('POST')
